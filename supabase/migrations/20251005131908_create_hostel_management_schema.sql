...
-- Alter meals table to add menu columns
ALTER TABLE meals 
ADD COLUMN IF NOT EXISTS day_menu_name text,
ADD COLUMN IF NOT EXISTS day_menu_image text,
ADD COLUMN IF NOT EXISTS night_menu_name text,
ADD COLUMN IF NOT EXISTS night_menu_image text;
...
CREATE POLICY "Users can view their hostel meals"
  ...
  USING (
    EXISTS (SELECT 1 FROM admins WHERE admins.id = meals.hostel_id AND admins.auth_id = auth.uid())
    OR EXISTS (SELECT 1 FROM members WHERE members.hostel_id = meals.hostel_id AND members.auth_id = auth.uid())
  );

CREATE POLICY "Admins can insert meals"
  ON meals FOR INSERT TO authenticated
  WITH CHECK (EXISTS (SELECT 1 FROM admins WHERE admins.id = meals.hostel_id AND admins.auth_id = auth.uid()));

CREATE POLICY "Admins can update meals"
  ON meals FOR UPDATE TO authenticated
  USING (EXISTS (SELECT 1 FROM admins WHERE admins.id = meals.hostel_id AND admins.auth_id = auth.uid()))
  WITH CHECK (EXISTS (SELECT 1 FROM admins WHERE admins.id = meals.hostel_id AND admins.auth_id = auth.uid()));
...
-- RLS Policies for meal_records
CREATE POLICY "Users can view their hostel meal records"
  ON meal_records FOR SELECT TO authenticated
  USING (
    EXISTS (SELECT 1 FROM meals m JOIN admins a ON a.id = m.hostel_id WHERE m.id = meal_records.meal_id AND a.auth_id = auth.uid())
    OR EXISTS (SELECT 1 FROM members WHERE members.id = meal_records.member_id AND members.auth_id = auth.uid())
    OR EXISTS (SELECT 1 FROM meals m JOIN members mem ON mem.hostel_id = m.hostel_id WHERE m.id = meal_records.meal_id AND mem.auth_id = auth.uid())
  );

CREATE POLICY "Admins can insert meal records"
  ON meal_records FOR INSERT TO authenticated
  WITH CHECK (EXISTS (SELECT 1 FROM meals m JOIN admins a ON a.id = m.hostel_id WHERE m.id = meal_records.meal_id AND a.auth_id = auth.uid()));

CREATE POLICY "Members can update their own meal records"
  ON meal_records FOR UPDATE TO authenticated
  USING (EXISTS (SELECT 1 FROM members WHERE members.id = meal_records.member_id AND members.auth_id = auth.uid()))
  WITH CHECK (EXISTS (SELECT 1 FROM members WHERE members.id = meal_records.member_id AND members.auth_id = auth.uid()));

CREATE POLICY "Admins can update meal records"
  ON meal_records FOR UPDATE TO authenticated
  USING (EXISTS (SELECT 1 FROM meals m JOIN admins a ON a.id = m.hostel_id WHERE m.id = meal_records.meal_id AND a.auth_id = auth.uid()))
  WITH CHECK (EXISTS (SELECT 1 FROM meals m JOIN admins a ON a.id = m.hostel_id WHERE m.id = meal_records.meal_id AND a.auth_id = auth.uid()));

CREATE POLICY "Admins can delete meal records"
  ON meal_records FOR DELETE TO authenticated
  USING (EXISTS (SELECT 1 FROM meals m JOIN admins a ON a.id = m.hostel_id WHERE m.id = meal_records.meal_id AND a.auth_id = auth.uid()));
...
CREATE POLICY "Users can view their hostel expenses"
  ...
  USING (
    EXISTS (SELECT 1 FROM admins WHERE admins.id = expenses.hostel_id AND admins.auth_id = auth.uid())
    OR EXISTS (SELECT 1 FROM members WHERE members.hostel_id = expenses.hostel_id AND members.auth_id = auth.uid())
  );

CREATE POLICY "Admins can insert expenses"
  ON expenses FOR INSERT TO authenticated
  WITH CHECK (EXISTS (SELECT 1 FROM admins WHERE admins.id = expenses.hostel_id AND admins.auth_id = auth.uid()));

CREATE POLICY "Admins can update expenses"
  ON expenses FOR UPDATE TO authenticated
  USING (EXISTS (SELECT 1 FROM admins WHERE admins.id = expenses.hostel_id AND admins.auth_id = auth.uid()))
  WITH CHECK (EXISTS (SELECT 1 FROM admins WHERE admins.id = expenses.hostel_id AND admins.auth_id = auth.uid()));
...
CREATE POLICY "Users can view their hostel notices"
  ...
  USING (
    EXISTS (SELECT 1 FROM admins WHERE admins.id = notices.hostel_id AND admins.auth_id = auth.uid())
    OR EXISTS (SELECT 1 FROM members WHERE members.hostel_id = notices.hostel_id AND members.auth_id = auth.uid())
  );

CREATE POLICY "Admins can insert notices"
  ON notices FOR INSERT TO authenticated
  WITH CHECK (EXISTS (SELECT 1 FROM admins WHERE admins.id = notices.hostel_id AND admins.auth_id = auth.uid()));

CREATE POLICY "Admins can update notices"
  ON notices FOR UPDATE TO authenticated
  USING (EXISTS (SELECT 1 FROM admins WHERE admins.id = notices.hostel_id AND admins.auth_id = auth.uid()))
  WITH CHECK (EXISTS (SELECT 1 FROM admins WHERE admins.id = notices.hostel_id AND admins.auth_id = auth.uid()));
